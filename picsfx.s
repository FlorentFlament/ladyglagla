...
picgum_fx_animation:
        ...
        jsr     set_palette
        move.l  4(a5),a3        ; next long is address of first
                                ; animation picture data
...
picdisplay_stretched_4colors:
        ...
        lea.l   32000(a4),a2            ; End of screen in a2
...
.picdisplay_loop:
        ...
        ;; a4 = base video memory addr

        ;; Compute index in picture data (d0)
        and.w   #$ff,d6         ; Cast d6 to word, to be used as an index
        ...
        asl.w   d0              ; a1 elements are words
        move.w  (a1,d0.w),d0    ; *80

        ;; Display a line
        REPT 20
        move.l  REPTN*4(a5,d0),REPTN*8(a4)
        ENDR

        ;;
        ;; Prepare next picture line
        ;;
        add.w   #160,a4         ; next video buffer line (a4)

        ;; Compute how many items to move forward in the displacement
        ;; table, i.e d3/d5
...
.d5_positive:
        ...
        cmp.l   a2,a4         ; display 200 lines
        blt     .picdisplay_loop

        movem.l (sp)+,a2-a4/d0/d5-d6
        rts
